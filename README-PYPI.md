...
* [cribl_control_plane_sdk_python](https://github.com/criblio/cribl_control_plane_sdk_python/blob/master/#criblcontrolplanesdkpython)
  ...
  * [Worker group and node URL scope](https://github.com/criblio/cribl_control_plane_sdk_python/blob/master/#worker-group-and-node-url-scope)
...
`server_url` is normally the leader host only (for example `http://localhost:9000` or `https://workspace-org.cribl.cloud`). If the URL has no path, the SDK appends `/api/v1`. You can still pass a full base URL including `/api/v1` (or another path) when needed.

Group-scoped APIs use `/m/{groupId}` and node-scoped `/w/{nodeId}` on top of that base. Use `with client.scoped(...)` with exactly one of `group_id`, `node_id`, or `leader=True` (leader resets scope for that block, e.g. deploy). You can also set `group_id` / `node_id` on `CriblControlPlane`, use `with_group` / `with_node`, or pass per-method `server_url`.
...
More detail: [`examples/example_onprem_configure_resources.py`](https://github.com/criblio/cribl_control_plane_sdk_python/blob/master/./examples/example_onprem_configure_resources.py).
