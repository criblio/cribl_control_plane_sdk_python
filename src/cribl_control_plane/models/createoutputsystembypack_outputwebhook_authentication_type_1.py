...
class CreateOutputSystemByPackGoogleAuthenticationMethod(
    str, Enum, metaclass=utils.OpenEnumMeta
):
...
class CreateOutputSystemByPackOutputGoogleCloudObservabilityTypedDict(TypedDict):
    ...
    google_auth_method: CreateOutputSystemByPackGoogleAuthenticationMethod
    r"""Choose Auto to use Google Application Default Credentials (ADC). Choose Secret to select or create a stored secret that references Google service account credentials."""
    pipeline: NotRequired[str]
    r"""Pipeline to process data before sending out to this output"""
    system_fields: NotRequired[List[str]]
...
class CreateOutputSystemByPackOutputGoogleCloudObservability(BaseModel):
    ...
    def serialize_google_auth_method(self, value):
        if isinstance(value, str):
            try:
                return models.CreateOutputSystemByPackGoogleAuthenticationMethod(value)
            except ValueError:
                return value
        return value
