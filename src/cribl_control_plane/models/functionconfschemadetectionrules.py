...
class LocalOverridesTypedDict(TypedDict):
    r"""Instance-level tuning applied after the rule set is merged. Managed by Cribl Security; not intended for direct editing."""
    ...
    r"""Full rule definitions authored here rather than delivered with the corpus."""
...
class FunctionConfSchemaDetectionRules(BaseModel):
    ...
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["localOverrides"])
        serialized = handler(self)
        m = {}
