"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from cribl_control_plane.types import BaseModel, UNSET_SENTINEL
import pydantic
from pydantic import model_serializer
from typing import Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class OrphanFileRecoveryTypeTypedDict(TypedDict):
    disabled: NotRequired[bool]
    r"""Periodically scan the staging directory for files not tracked by any Worker manifest to recover them"""
    period_min: NotRequired[float]
    r"""Minimum interval between reconciliation runs"""


class OrphanFileRecoveryType(BaseModel):
    disabled: Optional[bool] = None
    r"""Periodically scan the staging directory for files not tracked by any Worker manifest to recover them"""

    period_min: Annotated[Optional[float], pydantic.Field(alias="periodMin")] = None
    r"""Minimum interval between reconciliation runs"""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["disabled", "periodMin"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m


try:
    OrphanFileRecoveryType.model_rebuild()
except NameError:
    pass
